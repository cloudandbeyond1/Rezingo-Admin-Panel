...
// dummy data
import { orders, OrdersItemTypes } from "./UserData";

//name


const UsernameColumn = ({ row }: { row: any }) => {
    return (
      <>
        {row.original.name}{" "}
      </>
  ...
  return (
    <>
      <Link to="/apps/ecommerce/order/details" className="text-body fw-bold">
        #RZ_USER_{row.original.order_id}
      </Link>
    </>
  );
};
...
const UserDateColumn = ({ row }: { row: any }) => {
    return (
      <>
        {row.original.user_date}{" "}
      </>
    );
  };
  ...
  return (
    <>
      <h5>
        <span
          className={classNames("badge", {
            "bg-success": row.original.order_status === "Completed",
            "bg-danger": row.original.order_status === "Cancelled",
            "bg-info": row.original.order_status === "Shipped",
            "bg-warning": row.original.order_status === "On Going",
          })}
        >
